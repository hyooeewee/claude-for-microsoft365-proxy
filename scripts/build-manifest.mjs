...
async function main() {
  ...
  // Defaults: host and output file
  const host = args[0] || "office";
  const out = args[1] || "manifest.xml";

  // Load defaults from .env.local
  const envLocal = loadEnvLocal();
  const defaults = [];
  if (envLocal.GATEWAY_URL) defaults.push(`gateway_url=${envLocal.GATEWAY_URL}`);
  if (envLocal.GATEWAY_TOKEN) defaults.push(`gateway_token=${envLocal.GATEWAY_TOKEN}`);
  defaults.push("gateway_api_format=anthropic");
  defaults.push("gateway_auth_header=authorization");
  defaults.push("auto_connect=0");

  // User-provided pairs override defaults
  const userPairs = args.slice(2);
  const userKeys = new Set(userPairs.map((p) => {
    const eq = p.indexOf("=");
    return eq >= 0 ? p.slice(0, eq) : p;
  }));
  const pairs = [
    ...defaults.filter((p) => {
      const eq = p.indexOf("=");
  ...
  if (!manifestUrl || pairs.length === 0) {
    console.error("Usage: node build-manifest.mjs [office|outlook] [out] [key=value ...]");
